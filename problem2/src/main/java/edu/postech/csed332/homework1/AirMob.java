...
public class AirMob implements Monster {
    ...
    public Position move() {
        ...
        if (gameboard.isAirMob(x + 1, y)) canGo[0] = false;
        if (gameboard.isAirMob(x - 1, y)) canGo[2] = false;
        if (gameboard.isAirMob(x, y + 1)) canGo[1] = false;
        if (gameboard.isAirMob(x, y - 1)) canGo[3] = false;

        if (gameboard.isAirTower(x + 1, y + 1)) { canGo[0] = false; canGo[1] = false; }
        if (gameboard.isAirTower(x + 1, y - 1)) { canGo[0] = false; canGo[2] = false; }
        if (gameboard.isAirTower(x - 1, y + 1)) { canGo[1] = false; canGo[3] = false; }
